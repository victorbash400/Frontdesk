...
void AgentMikeRingBufferWrite(AgentMikeRingBuffer* buffer, const float* samples, uint32_t frameCount)
{
    ...
    if (frameCount > AGENT_MIKE_RING_FRAMES) {
        samples += (frameCount - AGENT_MIKE_RING_FRAMES) * AGENT_MIKE_CHANNELS;
        frameCount = AGENT_MIKE_RING_FRAMES;
    }
    if (writeFrame + frameCount - readFrame > AGENT_MIKE_RING_FRAMES) {
        readFrame = writeFrame + frameCount - AGENT_MIKE_RING_FRAMES;
        atomic_store_explicit(&buffer->readFrame, readFrame, memory_order_release);
    }
    for (uint32_t frame = 0; frame < frameCount; ++frame) {
        const uint32_t ringFrame = (uint32_t)((writeFrame + frame) % AGENT_MIKE_RING_FRAMES);
        memcpy(&buffer->samples[ringFrame * AGENT_MIKE_CHANNELS],
               &samples[frame * AGENT_MIKE_CHANNELS],
               sizeof(float) * AGENT_MIKE_CHANNELS);
    }
...
void AgentMikeRingBufferRead(AgentMikeRingBuffer* buffer, float* samples, uint32_t frameCount)
{
    ...
    for (uint32_t frame = 0; frame < readable; ++frame) {
        const uint32_t ringFrame = (uint32_t)((readFrame + frame) % AGENT_MIKE_RING_FRAMES);
        memcpy(&samples[frame * AGENT_MIKE_CHANNELS],
               &buffer->samples[ringFrame * AGENT_MIKE_CHANNELS],
               sizeof(float) * AGENT_MIKE_CHANNELS);
    }
    if (readable < frameCount) {
        memset(&samples[readable * AGENT_MIKE_CHANNELS], 0,
               sizeof(float) * (frameCount - readable) * AGENT_MIKE_CHANNELS);
    }
    readFrame += readable;
    atomic_store_explicit(&buffer->readFrame, readFrame, memory_order_release);
}
