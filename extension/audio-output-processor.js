class FrontDeskAudioOutputProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  enqueue(pcm) {
    ...
    for (const sample of pcm) {
      ...
      this.buffer[this.writeIndex] = sample / 32768;
  ...
  process(_inputs, outputs) {
    ...
    for (let index = 0; index < output.length; index += 1) {
      output[index] = this.currentSample + (this.nextSample - this.currentSample) * this.phase;
      this.phase += ratio;
      while (this.phase >= 1) {
        const sample = this.readSample();
        if (sample === undefined) {
          this.playing = false;
          this.port.postMessage({ type: this.turnComplete ? 'drained' : 'underrun' });
          this.turnComplete = false;
          return true;
        }
...
registerProcessor('front-desk-audio-output', FrontDeskAudioOutputProcessor);
