...
export const AuthTokenSection: React.FC<{}> = ({}) => {
  ...
  const configured = configuredAuthToken();
  ...
  return (
    <div className='auth-token-section'>
      <div className='auth-token-description'>
        Optional local connection token:
      </div>
      <div className='auth-token-container'>
        <code className='auth-token-code'>{authTokenCode(authToken)}</code>
        {!configured && <button className='auth-token-refresh' title='Generate new token' aria-label='Generate new token'onClick={onRegenerateToken}>{icons.refresh()}</button>}
...
export const getOrCreateAuthToken = (): string => {
  const configured = configuredAuthToken();
  if (configured) {
    localStorage.setItem('auth-token', configured);
    return configured;
  }
...
function configuredAuthToken(): string {
  return import.meta.env.FRONT_DESK_PLAYWRIGHT_EXTENSION_TOKEN || '';
}
