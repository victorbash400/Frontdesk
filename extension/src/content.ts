...
let sequence = 0;
let pendingSemantic: { message: string; intent: string } | undefined;
...
if (!(globalThis as typeof globalThis & { __frontDeskBrowserOverlayReady?: boolean }).__frontDeskBrowserOverlayReady) {
  ...
  window.addEventListener('message', event => {
    const message = event.data as { type?: string; message?: string; intent?: string };
    if (event.source !== window || message.type !== 'frontDeskBrowserIntent' || !message.message)
      return;
    pendingSemantic = { message: message.message, intent: message.intent || '' };
    showAction({ type: 'frontDeskBrowserAction', kind: 'move', label: message.message, intent: message.intent });
  });
}
...
function showAction(action: BrowserAction): void {
  ...
  const intent = overlay.shadowRoot!.querySelector<HTMLElement>('[data-front-desk-intent]')!;
  const next = coordinates(action);
  const target = targetAt(next.x, next.y, action.kind);
  const semantic = pendingSemantic;
  label.textContent = semantic?.message || action.label || actionText(action.kind, target);
  intent.textContent = semantic?.intent || action.intent || '';
  intent.hidden = !intent.textContent || intent.textContent === label.textContent;
...
function ensureOverlay(): HTMLElement {
  ...
  style.textContent = `
    *{box-sizing:border-box}
    ...
    .bubble{position:absolute;top:39px;left:25px;width:max-content;max-width:290px;overflow:hidden;padding:8px 10px;border:1px solid rgb(32 32 30 / 10%);border-radius:10px;background:rgb(248 248 246 / 94%);box-shadow:0 8px 24px rgb(0 0 0 / 12%);backdrop-filter:blur(14px);color:#242422;line-height:1.3}.label{display:block;overflow:hidden;font-size:12px;font-weight:590;text-overflow:ellipsis;white-space:nowrap}.intent{display:block;max-width:270px;margin-top:3px;overflow:hidden;color:#777772;font-size:10px;font-weight:500;text-overflow:ellipsis;white-space:nowrap}.intent[hidden]{display:none}
    .cursor[data-horizontal="left"] .bubble{right:25px;left:auto}.cursor[data-vertical="up"] .bubble{top:auto;bottom:39px}
