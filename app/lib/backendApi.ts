...
export async function backendApi(path: string, init: RequestInit = {}) {
  ...
  try {
    const response = await fetch(`${backendUrl.replace(/\/$/, "")}${path}`, {
      ...init,
      headers: { ...init.headers, "X-Front-Desk-Account": session.user.id, "X-Front-Desk-Internal-Secret": internalSecret },
      cache: "no-store",
    });
    const payload = await response.json().catch(() => ({ detail: "Backend request failed" })) as Record<string, unknown>;
    if (!response.ok) return Response.json({ error: typeof payload.detail === "string" ? payload.detail : "Backend request failed" }, { status: response.status });
