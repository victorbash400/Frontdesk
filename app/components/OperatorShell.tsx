...
type DialogState = { mode: "create" | "rename"; node?: FileSystemNode };
...
export function OperatorShell() {
  ...
  function createItem(name: string) {
    const root = navigation.destination.type === "location" && navigation.destination.location === "clients";
    const parentId = navigation.destination.type === "folder" ? navigation.destination.id : null;
    const node = fileSystem.createNode(name, root ? "client" : "folder", parentId);
    setDialog(undefined);
    setSelectedId(node.id);
  }
  ...
  return (
    <main className={styles.shell} onKeyDown={handleKeyboard} style={tagVariables} tabIndex={-1}>
      <Sidebar destination={navigation.destination} onNavigate={(location) => navigate({ type: "location", location })} />
      <section className={styles.explorer}>
        <Toolbar canGoBack={navigation.canGoBack} canGoForward={navigation.canGoForward} destination={navigation.destination} hasSelection={Boolean(selectedNode)} onBack={navigation.back} onCreate={() => setDialog({ mode: "create" })} onForward={navigation.forward} onInspectorToggle={() => setInspectorOpen((current) => !current)} onQueryChange={setQuery} onShare={() => selectedNode && fileSystem.updateNode(selectedNode.id, { shared: true })} onSortChange={setSort} onViewModeChange={setViewMode} query={query} sort={sort} title={title} viewMode={viewMode} />
