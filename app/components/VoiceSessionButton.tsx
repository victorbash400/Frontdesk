...
export function VoiceSessionButton({ hue, onStart, onStop, status }: { hue: number; onStart: () => void; onStop: () => void; status: VoiceStatus }) {
  ...
  return <button aria-label={active ? "Stop voice session" : "Start voice session"} className={styles.button} data-active={active} disabled={status === "connecting"} onClick={active ? onStop : onStart} type="button">{active ? <Square aria-hidden="true" /> : <Mic aria-hidden="true" style={{ color: `hsl(${(hue + 258) % 360} 62% 51%)` }} />}</button>;
}
