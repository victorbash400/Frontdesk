...
PROVIDERS = {
    ...
    "atlassian": MCPProvider(
        ...
        "read:me offline_access read:jira-work write:jira-work search:confluence read:page:confluence write:page:confluence read:comment:confluence write:comment:confluence",
    ),
...
async def finish_connection(session: Session, state: str, code: str) -> tuple[str, str]:
    ...
    async with httpx.AsyncClient(timeout=25) as client:
        token_response = await client.post(context["token_endpoint"], data=token_data, auth=request_auth)
        if token_response.is_error:
            raise RuntimeError(_oauth_error(token_response, "The plugin did not accept the connection."))
        tokens = token_response.json()
    access_token = tokens.get("access_token")
    if not isinstance(access_token, str) or not access_token:
        raise RuntimeError("The plugin did not return an access token.")
...
async def _discover(provider: MCPProvider) -> dict[str, str]:
    ...
    async with httpx.AsyncClient(timeout=20, follow_redirects=True) as client:
        challenge = await client.post(provider.server_url, json=INITIALIZE_BODY, headers=headers)
        resource_metadata_url = _resource_metadata_url(challenge.headers.get("www-authenticate", ""))
        if not resource_metadata_url:
            raise RuntimeError(f"{provider.id.title()} did not advertise OAuth connection metadata.")
    ...
    return {
        "resource": str(resource_metadata.get("resource") or provider.server_url),
