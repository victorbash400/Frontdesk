...
BUILTIN_SKILLS = (
    ...
    BuiltinSkill(
        ...
        "AquaLabs Customer Resolution",
        "Run an inbound customer complaint from email through confirmed resolution.",
...
If information is missing, reply in the same email thread with focused questions. If a call is appropriate, ask when the customer is available. Persist the exact email thread and case identity, then stop work with a declared external wait for that customer's reply; never poll and never keep a worker alive while waiting.

When the matching reply arrives, resume the same case. Schedule a Google Meet for the confirmed time, send the invitation, and have the meeting worker join the exact meeting. The meeting agent waits silently until the customer speaks, uses the case goal and stored documents, and delegates bounded investigation or repair work to the coordinator when necessary.
